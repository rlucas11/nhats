...
summarizeR(cor(dataSelect[,12:22], use="pair"), 1)
...
y10 ~~ 0*y10 
y11 ~~ 0*y11 
 ## Stable Trait/Random Intercepts for X 
ri_x =~ 1*x1 
ri_x =~ 1*x2 
ri_x =~ 1*x3 
ri_x =~ 1*x4 
...
## Trait Variance 
ri_x ~~ lower(0)*ri_x 
 ## Stable Trait/Random Intercepts for Y 
ri_y =~ 1*y1 
ri_y =~ 1*y2 
ri_y =~ 1*y3 
ri_y =~ 1*y4 
...
arvy > 0 
arv2y > 0 
arv2x == (1-a^2)*arvx - 2*a*d*cor_xy - d^2*arvy 
 
arv2y == (1-b^2)*arvy - 2*b*c*cor_xy - c^2*arvx
"

startsLFit <- lavaan(starts, data=dataSelect, estimator="MLR", missing="fiml")
summary(startsL, fit.measures=TRUE)
